defmodule Enrolmentform.Accounts do
  @moduledoc """
  The Accounts context.
  """

  import Ecto.Query, warn: false
  alias Enrolmentform.Repo

  alias Enrolmentform.Accounts.{Admin, AdminToken, AdminNotifier}
  ...
  See `Enrolmentform.Accounts.Admin.email_changeset/3` for a list of supported options.
  ...
  defp admin_email_multi(admin, email, context) do
    changeset = Admin.email_changeset(admin, %{email: email})
  ...
  See `Enrolmentform.Accounts.Admin.password_changeset/3` for a list of supported options.
  ...
  Returns the updated admin, as well as a list of expired tokens.
  ...
  def update_admin_password(admin, attrs) do
    ...
    |> case do
      {:ok, admin, expired_tokens} -> {:ok, admin, expired_tokens}
  ...
  defp update_admin_and_delete_all_tokens(changeset) do
    %{data: %Admin{} = admin} = changeset

    with {:ok, %{admin: admin, tokens_to_expire: expired_tokens}} <-
           Ecto.Multi.new()
           |> Ecto.Multi.update(:admin, changeset)
           |> Ecto.Multi.all(:tokens_to_expire, AdminToken.by_admin_and_contexts_query(admin, :all))
           |> Ecto.Multi.delete_all(:tokens, fn %{tokens_to_expire: tokens_to_expire} ->
             AdminToken.delete_all_query(tokens_to_expire)
           end)
           |> Repo.transaction() do
