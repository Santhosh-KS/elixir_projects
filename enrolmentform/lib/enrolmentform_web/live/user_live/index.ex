defmodule EnrolmentformWeb.UserLive.Index do
  ...
  def render(assigns) do
    ...
    <Layouts.app flash={@flash}>
      ...
      >
        ...
        <:col :let={{_id, user}} label="LinkedInUrl">{user.linkedInUrl}</:col>
        <:action :let={{_id, user}}>
          <div class="sr-only">
  ...
  def mount(_params, _session, socket) do
    {:ok,
     ...
     |> stream(:users, Courses.list_users())}
  end

  @impl true
  def handle_event("delete", %{"id" => id}, socket) do
    user = Courses.get_user!(id)
    {:ok, _} = Courses.delete_user(user)
