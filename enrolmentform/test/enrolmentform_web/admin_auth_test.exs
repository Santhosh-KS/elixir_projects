defmodule EnrolmentformWeb.AdminAuthTest do
  use EnrolmentformWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Enrolmentform.Accounts
  alias Enrolmentform.Accounts.Scope
  alias EnrolmentformWeb.AdminAuth

  import Enrolmentform.AccountsFixtures

  @remember_me_cookie "_enrolmentform_web_admin_remember_me"
  @remember_me_cookie_max_age 60 * 60 * 24 * 14

  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, EnrolmentformWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_admin/3" do
    ...
    test "redirects to settings when admin is already logged in", %{conn: conn, admin: admin} do
      ...
      assert redirected_to(conn) == "/admins/settings"
    end
  ...
  describe "logout_admin/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      EnrolmentformWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        EnrolmentformWeb.Endpoint.subscribe("admins_sessions:#{Base.url_encode64(token)}")
      end
